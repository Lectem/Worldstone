...
bool DT1::initDecoder(StreamPtr&& streamPtr)
{
    ...
    if (stream && stream->good()) { return readHeaders(); }
    return false;
}
...
bool DT1::readHeaders()
{
    ...
    static_assert(Utils::StaticCheckSize<DT1::Header, 276>(),
                  "The DT1 file header size is 276 bytes");
    stream->read(&header, sizeof(header));
    if (stream->fail()) return false;

    if (header.version != 7) return false;
    tileHeaders.resize(header.numTiles);
    static_assert(Utils::StaticCheckSize<DT1::Tile::Header, 96>(),
                  "The tiles header size is 96 bytes");
